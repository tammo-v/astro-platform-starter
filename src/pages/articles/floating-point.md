...
No expansion is performed. No approximation is introduced. The result is a new ISE, composed of ISUs and nested ISEs—such as `cos(x)` and `sin(x)`—that remains fully symbolic and structurally intact. It can be reused, composed, or transformed—without ever being flattened.
